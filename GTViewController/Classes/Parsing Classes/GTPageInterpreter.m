...
- (id)createLabelFromElement:(TBXMLElement *)element parentTextAlignment:(UITextAlignment)panelAlign xPos:(CGFloat)xpostion yPos:(CGFloat)ypostion container:(UIView *)container {
	...
	if (element != nil) {
		...
		if (textalign) {
			...
			else if ([textalign isEqual:kAlignment_center])	{textAlignment		= NSTextAlignmentRight;}
	...
	if (element) {
		...
		if (subTitle_element) {
			...
			if (align) {
				...
				else if ([align isEqual:kAlignment_center])	{textAlignment		= NSTextAlignmentRight;}
			...
			UILabel	*peekPanelArrow = [self createLabelWithFrame:peekPanelArrowFrame autoResize:NO text:@"▼" color:[UIColor whiteColor] bgColor: [UIColor clearColor] alpha:1.0 alignment:NSTextAlignmentRight font:kFont_label size:8];
